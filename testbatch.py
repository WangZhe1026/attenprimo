...
from models.utils import DiffNNSearch, FMAP_SOLVERS
from models.utils import to_numpy, validate_gradient, validate_tensor, fmap_to_image, bslice, diff_zoomout
from models.utils import orthogonality_s_loss
...
from primodel.prism import PrismDecoder
from primodel.loss import PrismRegularizationLoss
...
class Trainer(object):
    ...
    def _init_train(self, phase='train'):
        ...
        parameters += list(self.primo_model.parameters())
    ...
    def _train_epoch(self, epoch, phase='train'):
        ...
        fmit = cfg['feat_model.in_type'] # 定义了数据类型 'vertices'、'edges'、'faces'等等?
...
# 打开 CSV 文件以追加模式写入
            ...
            loss_interim = 0
            for sidx, SD in enumerate(spectral_dims):
                loss_interim = loss_interim + get_weight(SD) * orthogonality_s_loss(fmaps01_init[sidx])
            loss_interim /= len(spectral_dims)

            loss_final = orthogonality_s_loss(fmap01_final)


            # todo: add primodel loss

            # primo_loss_fn = PrismRegularizationLoss()
            # transformed_prism = self.primo_model(fmap01_final, batch_data['rotations'], batch_data['verts'], batch_data['faces'], batch_data['normals'])            
            # loss_primo = primo_loss_fn(transformed_prism, rotations, verts, faces, normals)
            loss = cfg['loss.final_weight'] * loss_final + cfg['loss.interim_weight'] * loss_interim #+ loss_primo
            ...
            if (iter_idx + 1) % cfg['log_step'] == 0 or (iter_idx + 1) == num_iters:
                ...
                attns01_tab = np.stack((np.asarray(spectral_dims), np.mean(to_numpy(attns01), axis=0)), axis=1)
                attns01_tab = wandb.Table(data=attns01_tab, columns=['dim', 'attn'])
                log_dict['attns01'] = wandb.plot.line(attns01_tab, 'dim', 'attn')
            ...
            if (iter_idx + 1) % cfg['optim.accum_step'] == 0 or (iter_idx + 1) == num_iters:
                if validate_gradient(self.feat_model) and \
                   ...
                   validate_gradient(self.nnsearcher) :
                    for m in [self.feat_model, self.san_model, self.nnsearcher]:
                        torch.nn.utils.clip_grad_value_(m.parameters(), cfg['optim.grad_clip'])
                    self.optimizer.step()
                else:
