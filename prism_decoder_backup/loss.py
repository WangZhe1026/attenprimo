...
class PrismRegularizationLoss(nn.Module):
    ...
    def forward(self, transformed_prism, rotations, verts, faces, normals):
        ...
        # compute the energy
        # 这部分是在计算PriMo能量。f_p1_**和f_p2_**应该分别是第一个面和第二个面上的4个顶点坐标。通过torch.stack将它们堆叠起来,得到A和B两个张量,形状为(n_edges, 4, 3)。
        # 然后计算A和B的差,并将差值传入compute_energy函数计算能量,得到一个形状为(n_edges,)的能量张量energy
        ...
        # compute weight
        # 这部分是在计算每个能量项的权重。首先从face_areas中取出相应面的面积area1和area2。然后计算每个边的长度的平方,再除以相邻两个面的面积之和,
        # 得到一个形状为(n_edges,)的权重张量weight。最后将能量energy与权重相乘。
