...
def run_exp(cfg, test_root, out_root):
    ...
    if Path(out_root + '.pkl').is_file():
        ...
        for pid in range(len(dset)):
            ...
            match010_ref = np.stack([corr0, pmap10_ref[corr1]], axis=-1) # match010_ref 是一个二维数组 第一行是第一个形状的预测点，第二行是第二个形状的真实点gt
            match010_ref = np.ravel_multi_index(match010_ref.T, dims=[num_verts0, num_verts0]) #todo: xxxxyyyy => xyxyxyxy
            geoerrs_ref = np.take(geodist0, match010_ref) / sqrt_area0 # todo :这个geoerrs_ref是不是就是tttt\cougar_01.mat里存的dist
